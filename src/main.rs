...
//!    - optional digital VU meter at the bottom
//!
//! Design rule:
...
/// Computes a simple RMS level from the recorded WAV file.
///
/// Assumes:
/// - 16-bit little-endian PCM
/// - standard 44-byte WAV header
///
/// Returns a normalized level in the range 0.0..=1.0 after a small UI boost.
...
fn run_recognition_loop(
    ...
    while running.load(Ordering::SeqCst) {
        ...
        // This happens before SongRec so the UI still gets live level updates
        // even when recognition returns no match.
...
fn run_display_loop(
    ...
    while running.load(Ordering::SeqCst) {
        ...
        // Optional digital VU meter at the bottom of the scene.
